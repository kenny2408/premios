...
# Se testean => Models || Vistas
class QuestionModelTests(TestCase):

  def setUp(self):
    self.question = Question(question_text = 'Quién es el mejor Course Director?')

  def test_was_published_rencently_with_future_questions(self):
    '''was_published_rencently returns False for questions whose pub_date is in the future'''
...
class QuestionIndexViewTests(TestCase):
  ...
  def test_two_future_question(self):
    '''
    The questions with the pub_date in the future are not displayed on the index page
    '''
...
class QuestionDetailViewTests(TestCase):
  ...
  def test_future_question(self):
    '''
    The detail view of question with a pub_date in the future return a
    404 "error not found"
    '''
    future_question = create_question(question_text='Future question', days=30)
